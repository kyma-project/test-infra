...
| **always_run**              | A parameter that defines if a PR must trigger the job automatically. They are mutually exclusive with `run_if_changed`. If neither `run_if_changed` nor `always_run` are set, the job can only be triggered manually by a comment.                                                                         |
