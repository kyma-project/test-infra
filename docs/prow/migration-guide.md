...
| **always_run**              | A parameter that defines if a PR must trigger the job automatically. `always_run` and `run_if_changed` are mutually exclusive. If you do not set one of them, you can only trigger the job manually by a comment.                                                                       |
