...
> **NOTE:** The bucket for storing logs is also used in Prow by Spyglass to display the logs and it needs to be publicly accessible. Assign `Storage Object Viewer` and `Storage Legacy Bucket Reader` roles to `allUsers`.
