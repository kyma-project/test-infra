...
1. Create the configuration file.
   ...
   Go to `templates/data` and add a new YAML file (`<COMPONENT NAME>-data.yaml`) with Prow job details to the `render` list under the `templates` section.
   ...
   For **not-component job**, instead of `path`, you must specify Prow job `name`. See an example of the `skr-aws-upgrade-integration-dev` template:
   ```yaml
   templates:
      - fromTo:
...
5. Merge the changes.
   ...
   Create a PR with your changes in the `test-infra` repository including the job files generated by the Render Templates tool.
