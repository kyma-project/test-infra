...
gcloud compute scp --quiet install.sh kyma-integration-test-${RANDOM_ID}:~/install.sh 

echo "Triggering the installation script..."
gcloud compute ssh --quiet kyma-integration-test-${RANDOM_ID} -- ./install.sh --repo-owner $REPO_OWNER --branch $PULL_BASE_REF --pr-number $PULL_NUMBER