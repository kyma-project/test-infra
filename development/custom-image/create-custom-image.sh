...
if [[ $testK3d == true ]]; then
    ...
    cli_latest=$(curl -s https://api.github.com/repos/kyma-project/cli/releases/latest | grep tag_name | cut -d '"' -f 4)
    utils::ssh_to_vm_with_script -z "${ZONE}" -n "${VM_NAME}" -c "curl -Lo kyma.tar.gz https://github.com/kyma-project/cli/releases/download/${cli_latest}/kyma_Linux_x86_64.tar.gz && tar -zxvf kyma.tar.gz && chmod +x kyma"
