...
#shellcheck disable=SC2088
utils::ssh_to_vm_with_script "${ZONE}" "${VM_NAME}" "./install-deps-debian.sh"

log::info "Clearing $VM_NAME machine-id ..."
#shellcheck disable=SC2088
utils::ssh_to_vm_with_script "${ZONE}" "${VM_NAME}" "sudo sh -c 'echo "" > /etc/machine-id'"
utils::ssh_to_vm_with_script "${ZONE}" "${VM_NAME}" "sudo sh -c 'echo "" > /var/lib/dbus/machine-id'"
utils::ssh_to_vm_with_script "${ZONE}" "${VM_NAME}" "sudo sh -c 'echo \"RateLimitInterval=30s\" > /etc/systemd/journald.conf'"
utils::ssh_to_vm_with_script "${ZONE}" "${VM_NAME}" "sudo sh -c 'echo \"RateLimitBurst=1500\" >> /etc/systemd/journald.conf'"
utils::send_to_vm "${ZONE}" "$VM_NAME" "$CURRENT_DIR/resources/dbus-1_system-local.conf" "/tmp/system-local.conf"
utils::ssh_to_vm_with_script "${ZONE}" "${VM_NAME}" "sudo sh -c 'mv /tmp/system-local.conf /etc/dbus-1/system-local.conf'"
