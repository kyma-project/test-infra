...
func TestGovernanceJobPresubmit(t *testing.T) {
	// WHEN
	jobConfig, err := tester.ReadJobConfig("./../../../../prow/jobs/website/website-governance.yaml")
	// THEN
	require.NoError(t, err)

	assert.Len(t, jobConfig.Presubmits, 1)
	presubmits, ex := jobConfig.Presubmits["kyma-project/website"]
	assert.True(t, ex)
	assert.Len(t, presubmits, 1)

	expName := "website-governance"
...
func TestGovernanceJobPeriodic(t *testing.T) {
	...
	tester.AssertThatHasExtraRefs(t, actualPeriodic.JobBase.UtilityConfig, []string{"test-infra", "website"})
	...
	repositoryDirArg := fmt.Sprintf("--repository-dir %s/website", tester.KymaProjectDir)
	assert.Equal(t, []string{"--repository website", repositoryDirArg, "--full-validation true"}, actualPeriodic.Spec.Containers[0].Args)
}
