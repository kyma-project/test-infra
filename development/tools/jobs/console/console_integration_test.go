...
func TestCoreJobPresubmit(t *testing.T) {
	...
	assert.Equal(t, []string{"kyma-project"}, actualPresubmit.ExtraRefs[1].Org)
	assert.Equal(t, []string{"console"}, actualPresubmit.ExtraRefs[1].Repo)
	assert.Equal(t, []string{"master"}, actualPresubmit.ExtraRefs[1].BaseRef)
	assert.Equal(t, []string{"github.com/kyma-project/console"}, actualPresubmit.ExtraRefs[1].PathAlias)
...
func TestCoreJobPostsubmit(t *testing.T) {
	...
	assert.Equal(t, []string{"kyma-project"}, actualPost.ExtraRefs[1].Org)
	assert.Equal(t, []string{"console"}, actualPost.ExtraRefs[1].Repo)
	assert.Equal(t, []string{"master"}, actualPost.ExtraRefs[1].BaseRef)
	assert.Equal(t, []string{"github.com/kyma-project/console"}, actualPost.ExtraRefs[1].PathAlias)
