...
func TestBundlesGovernanceJobPresubmit(t *testing.T) {
	...
	presubmits, ex := jobConfig.Presubmits["kyma-project/bundles"]
	assert.True(t, ex)
...
func TestBundlesGovernanceJobPeriodic(t *testing.T) {
	...
	expName := "bundles-governance-nightly"
	...
	tester.AssertThatHasExtraRefs(t, actualPeriodic.JobBase.UtilityConfig, []string{"test-infra", "bundles"})
	...
	repositoryDirArg := fmt.Sprintf("%s/bundles", tester.KymaProjectDir)
	assert.Equal(t, []string{"--repository", "bundles", "--repository-dir", repositoryDirArg, "--full-validation", "true"}, actualPeriodic.Spec.Containers[0].Args)
}
