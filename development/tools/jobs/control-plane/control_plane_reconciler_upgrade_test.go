...
const reconcilerUpgradeTestJobPath = "./../../../../prow/jobs/control-plane/control-plane-reconciler-upgrade-kyma2-latest-to-main-gardener.yaml"

func TestReconcilerJobsPresubmitUpgradeE2E(t *testing.T) {
	// WHEN
	jobConfig, err := tester.ReadJobConfig(reconcilerUpgradeTestJobPath)
	// THEN
	require.NoError(t, err)

	kymaPresubmits := jobConfig.AllStaticPresubmits([]string{"kyma-project/control-plane"})
	expName := "pre-main-control-plane-reconciler-upgrade-kyma2-latest-to-main-gardener"
	...
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-project/test-infra/prow/scripts/cluster-integration/reconciler-upgrade-kyma2-latest-to-main-gardener.sh"}, actualPresubmit.Spec.Containers[0].Command)
