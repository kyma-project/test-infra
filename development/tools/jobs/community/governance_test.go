...
func TestGovernanceJobPresubmit(t *testing.T) {
	...
	assert.Equal(t, []string{"community"}, actualPresubmit.Spec.Containers[0].Args)
}
...
func TestGovernanceJobPeriodic(t *testing.T) {
	...
	assert.Equal(t, "github.com/kyma-project/community", actualPeriodic.PathAlias)
	...
	assert.Equal(t, []string{"community", "true"}, actualPeriodic.Spec.Containers[0].Args)
}
