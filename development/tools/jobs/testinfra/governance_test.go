...
func TestGovernanceJobPresubmit(t *testing.T) {
	...
	assert.Equal(t, []string{"test-infra"}, actualPresubmit.Spec.Containers[0].Args)
}
...
func TestGovernanceJobPeriodic(t *testing.T) {
	...
	assert.Equal(t, "github.com/kyma-project/test-infra", actualPeriodic.PathAlias)
	...
	assert.Equal(t, []string{"test-infra", "true"}, actualPeriodic.Spec.Containers[0].Args)
}
