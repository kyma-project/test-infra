...
func TestGovernanceJobPresubmit(t *testing.T) {
	...
	presubmits, ex := jobConfig.Presubmits["kyma-project/examples"]
	assert.True(t, ex)
	assert.Len(t, presubmits, 1)

	expName := "examples-governance"
	...
	assert.Equal(t, []string{"examples"}, actualPresubmit.Spec.Containers[0].Args)
}
...
func TestGovernanceJobPeriodic(t *testing.T) {
	...
	assert.Equal(t, "github.com/kyma-project/examples", actualPeriodic.PathAlias)
	...
	assert.Equal(t, []string{"examples", "true"}, actualPeriodic.Spec.Containers[0].Args)
}
