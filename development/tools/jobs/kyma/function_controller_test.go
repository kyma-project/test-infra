...
func TestKnativeFunctionControllerAcceptanceReleases(t *testing.T) {
	...
	for _, currentRelease := range tester.GetKymaReleasesSince(tester.Release14) {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], tester.GetReleaseJobName("kyma-components-function-controller", currentRelease), currentRelease.Branch())
			...
			tester.AssertThatExecGolangBuildpack(t, actualPresubmit.JobBase, tester.ImageGolangKubebuilderBuildpackLatest, "/home/prow/go/src/github.com/kyma-project/kyma/components/function-controller")
...
func TestKnativeFunctionControllerAcceptanceJobsPresubmit(t *testing.T) {
	...
	// THEN
	require.NoError(t, err)
	actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], "pre-master-kyma-components-function-controller", "master")
	require.NotNil(t, actualPresubmit)
	...
	assert.Equal(t, "^components/function-controller/", actualPresubmit.RunIfChanged)
...
func TestKnativeFunctionControllerAcceptanceJobPostsubmit(t *testing.T) {
	...
	expName := "post-master-kyma-components-function-controller"
	...
	assert.Equal(t, "^components/function-controller/", actualPost.RunIfChanged)
	assert.Equal(t, tester.ImageGolangKubebuilderBuildpackLatest, actualPost.Spec.Containers[0].Image)
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-project/test-infra/prow/scripts/build.sh"}, actualPost.Spec.Containers[0].Command)
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-project/kyma/components/function-controller"}, actualPost.Spec.Containers[0].Args)
}
