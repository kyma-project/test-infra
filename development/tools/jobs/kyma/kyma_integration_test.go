...
func TestKymaGKEBackupJobsReleases(t *testing.T) {
	for _, currentRelease := range releases.GetKymaReleasesSince(releases.Release14) {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			assert.Len(t, actualPresubmit.Spec.Containers, 1)
			testContainer := actualPresubmit.Spec.Containers[0]
			assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", testContainer.Image)
			assert.Len(t, testContainer.Command, 1)
			tester.AssertThatSpecifiesResourceRequests(t, actualPresubmit.JobBase)
		})
	}
...
func TestKymaBackupTestJobPresubmit(t *testing.T) {
	...
	tester.AssertThatHasPresets(t, actualJob.JobBase, preset.KymaBackupRestoreBucket, preset.KymaBackupCredentials, preset.GCProjectEnv, preset.BuildPr,
		preset.SaGKEKymaIntegration, "preset-weekly-github-integration")
	tester.AssertThatHasExtraRefTestInfra(t, actualJob.JobBase.UtilityConfig, "master")
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", actualJob.Spec.Containers[0].Image)
...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", orphanedAZStorageAccountsCleaner.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", nightlyPeriodic.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", weeklyPeriodic.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", nightlyAksPeriodic.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", backupRestorePeriodic.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/pr/kyma-cluster-infra:PR-1477", loadTestPeriodic.Spec.Containers[0].Image)
