...
func TestKymaGKEBackupJobsReleases(t *testing.T) {
	for _, currentRelease := range releases.GetKymaReleasesSince(releases.Release14) {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			assert.Len(t, actualPresubmit.Spec.Containers, 1)
			testContainer := actualPresubmit.Spec.Containers[0]
			assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", testContainer.Image)
			assert.Len(t, testContainer.Command, 1)
			tester.AssertThatSpecifiesResourceRequests(t, actualPresubmit.JobBase)
		})
	}
...
func TestKymaIntegrationJobsPresubmitXipDebug(t *testing.T) {
	expPresets := []preset.Preset{preset.GCProjectEnv, preset.BuildPr, preset.DindEnabled, preset.KymaGuardBotGithubToken, "preset-sa-gke-kyma-integration", "preset-gc-compute-envs", "preset-docker-push-repository-gke-integration"}
	...
	// when
	actualJob := tester.FindPresubmitJobByNameAndBranch(jobConfig.Presubmits["kyma-project/kyma"], "pre-master-kyma-gke-integration-xip-debug", "master")
	require.NotNil(t, actualJob)
...
func TestKymaBackupTestJobPresubmit(t *testing.T) {
	...
	tester.AssertThatHasPresets(t, actualJob.JobBase, preset.KymaBackupRestoreBucket, preset.KymaBackupCredentials, preset.GCProjectEnv, preset.BuildPr,
		preset.SaGKEKymaIntegration, "preset-weekly-github-integration")
	tester.AssertThatHasExtraRefTestInfra(t, actualJob.JobBase.UtilityConfig, "master")
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", actualJob.Spec.Containers[0].Image)
...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", orphanedAZStorageAccountsCleaner.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", nightlyPeriodic.Spec.Containers[0].Image)
	...
	assert.Len(t, nightlyPeriodic.Spec.Containers[0].Env, 7)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", weeklyPeriodic.Spec.Containers[0].Image)
	...
	assert.Len(t, weeklyPeriodic.Spec.Containers[0].Env, 7)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", nightlyAksPeriodic.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", backupRestorePeriodic.Spec.Containers[0].Image)
	...
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-cluster-infra:v20190927-2687a99", loadTestPeriodic.Spec.Containers[0].Image)
