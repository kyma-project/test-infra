...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, []string{"-c", "development/clusters-cleanup.sh -project=${CLOUDSDK_CORE_PROJECT} -dryRun=false -whitelisted-clusters=kyma-prow,workload-kyma-prow,nightly,weekly,service-catalog-nightly"}, clustersCleanerPeriodic.Spec.Containers[0].Args)
	...
	tester.AssertThatContainerHasEnv(t, cont, "TEST_RESULT_WINDOW_TIME", "6h")
