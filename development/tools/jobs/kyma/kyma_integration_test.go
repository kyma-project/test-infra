...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, []string{"-c", "/prow-tools/diskscollector -project=${CLOUDSDK_CORE_PROJECT} -dryRun=false -diskNameRegex='^gke|weekly|nightly|restore'"}, disksCleanerPeriodic.Spec.Containers[0].Args)
