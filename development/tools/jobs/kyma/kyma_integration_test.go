...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, []string{"-c", "/prow-tools/vmscollector -project=${CLOUDSDK_CORE_PROJECT} -vmNameRegexp='.*-integration-test-.*|busola-integration-test-.*' -jobLabelRegexp='.*-integration$|busola-integration-test' -dryRun=false"}, vmsCleanerPeriodic.Spec.Containers[0].Args)
	...
	assert.Equal(t, tester.ImageProwToolsCurrent, githubIssuesPeriodic.Spec.Containers[0].Image)
