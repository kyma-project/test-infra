...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, []string{"-c", "development/clusters-cleanup.sh -project=${CLOUDSDK_CORE_PROJECT} -dryRun=false -whitelisted-clusters=kyma-prow,workload-kyma-prow,nightly,weekly -strategy=time"}, clustersCleanerPeriodic.Spec.Containers[0].Args)
