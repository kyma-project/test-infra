...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, []string{"-c", "/prow-tools/diskscollector -project=${CLOUDSDK_CORE_PROJECT} -dryRun=false -diskNameRegex='^gke-upgrade|gke-provisioner|gke-backup|weekly|nightly|gke-central|gke-minio|restore|gke-release|gke-gke|gke-test|gke-cli'"}, disksCleanerPeriodic.Spec.Containers[0].Args)
