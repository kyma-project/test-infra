...
func TestKymaIntegrationJobPeriodics(t *testing.T) {
	...
	assert.Equal(t, []string{"-c", "/prow-tools/vmscollector -project=${CLOUDSDK_CORE_PROJECT} -vmNameRegexp='kyma-upgrade-test-.*|.*-integration-test-.*|busola-.*-test-.*|.*-upgrade-test-.*' -jobLabelRegexp='.*-integration$|busola-integration-test-k3s|.*-upgrade-k3d-.*|kyma-upgrade|busola-.*-test' -dryRun=false"}, vmsCleanerPeriodic.Spec.Containers[0].Args)
