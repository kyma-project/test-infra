...
func TestPresubmitDevelopmentArtifacts(t *testing.T) {
	...
	tester.AssertThatHasPresets(t, job.JobBase, preset.DindEnabled, preset.DockerPushRepoKyma, "preset-kyma-development-artifacts-bucket", preset.GcrPush, preset.BuildPr)
...
func TestPostsubmitDevelopmentArtifcts(t *testing.T) {
	...
	tester.AssertThatHasExtraRefTestInfra(t, job.UtilityConfig, "master")
	tester.AssertThatHasPresets(t, job.JobBase, preset.DindEnabled, preset.DockerPushRepoKyma, "preset-kyma-development-artifacts-bucket", preset.GcrPush, preset.BuildMaster)
	require.Len(t, job.Spec.Containers, 1)
	cont := job.Spec.Containers[0]
	assert.Equal(t, tester.ImageGolangBuildpack1_12, cont.Image)
	require.Len(t, cont.Command, 1)
