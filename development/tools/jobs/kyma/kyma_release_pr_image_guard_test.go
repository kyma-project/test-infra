...
func TestKymaReleasePrImageGuard(t *testing.T) {
	...
	actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], "pre-release-pr-image-guard")
	assert.True(t, actualPresubmit.RunsAgainstBranch("release-1.6"))
	assert.True(t, actualPresubmit.RunsAgainstBranch("release-2.3"))
