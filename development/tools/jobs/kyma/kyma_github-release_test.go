...
func TestKymaGithubReleaseJobPostsubmit(t *testing.T) {
	...
	for _, currentRelease := range releases.GetAllKymaReleases() {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			assert.Equal(t, []string{"-c", "/home/prow/go/src/github.com/kyma-project/test-infra/development/github-release.sh -targetCommit=${PULL_BASE_REF} -githubRepoOwner=${REPO_OWNER} -githubRepoName=${REPO_NAME} -githubAccessToken=${BOT_GITHUB_TOKEN} -releaseVersionFilePath=VERSION"}, actualPostsubmit.Spec.Containers[0].Args)
		})
	}
}
