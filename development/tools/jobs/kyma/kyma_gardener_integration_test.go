...
func TestKymaGardenerAzureIntegrationJobPeriodics(t *testing.T) {
	...
	tester.AssertThatHasPresets(t, job.JobBase, preset.GardenerAzureIntegration, preset.KymaCLIStable, preset.KymaKeyring, preset.KymaEncriptionKey)
	tester.AssertThatHasExtraRefs(t, job.JobBase.UtilityConfig, []string{"test-infra", "kyma"})
	assert.Equal(t, "eu.gcr.io/kyma-project/test-infra/kyma-integration:v20200401-531ac16b", job.Spec.Containers[0].Image)
