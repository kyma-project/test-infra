...
func TestApplicationRegistryReleases(t *testing.T) {
	...
	for _, currentRelease := range tester.GetAllKymaReleases() {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], tester.GetReleaseJobName("kyma-components-application-registry", currentRelease), currentRelease.Branch())
			...
			tester.AssertThatExecGolangBuildpack(t, actualPresubmit.JobBase, tester.ImageGolangBuildpackLatest, "/home/prow/go/src/github.com/kyma-project/kyma/components/application-registry")
		})
	}
}
...
func TestApplicationRegistryJobsPresubmit(t *testing.T) {
	...
	// THEN
	require.NoError(t, err)
	actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], "pre-master-kyma-components-application-registry", "master")
	require.NotNil(t, actualPresubmit)
...
func TestApplicationRegistryJobPostsubmit(t *testing.T) {
	...
	expName := "post-master-kyma-components-application-registry"
	...
	assert.Equal(t, "^components/application-registry/", actualPost.RunIfChanged)
	assert.Equal(t, tester.ImageGolangBuildpackLatest, actualPost.Spec.Containers[0].Image)
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-project/test-infra/prow/scripts/build.sh"}, actualPost.Spec.Containers[0].Command)
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-project/kyma/components/application-registry"}, actualPost.Spec.Containers[0].Args)
}
