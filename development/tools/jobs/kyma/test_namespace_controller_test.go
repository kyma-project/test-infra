...
func TestNamespaceControllerTestsReleases(t *testing.T) {
	...
	for _, currentRelease := range tester.GetAllKymaReleases() {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			// THEN
			require.NoError(t, err)
			actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], tester.GetReleaseJobName("kyma-tests-test-namespace-controller", currentRelease), currentRelease.Branch())
			require.NotNil(t, actualPresubmit)
			assert.False(t, actualPresubmit.SkipReport)
			assert.True(t, actualPresubmit.Decorate)
			assert.Equal(t, "github.com/kyma-project/kyma", actualPresubmit.PathAlias)
