...
func TestKymaGKESeparateCompassIntegrationPresubmit(t *testing.T) {
	// given
	jobConfig, err := tester.ReadJobConfig("./../../../../prow/jobs/kyma/kyma-gke-separate-compass-integration.yaml")
	require.NoError(t, err)

	// when
	actualJob := tester.FindPresubmitJobByNameAndBranch(jobConfig.AllStaticPresubmits([]string{"kyma-project/kyma"}), "pre-master-kyma-gke-separate-compass-integration", "master")
	...
	assert.Equal(t, "${KYMA_PROJECT_DIR}/test-infra/prow/scripts/cluster-integration/kyma-gke-separate-compass-integration.sh", compassCont.Args[1])
	tester.AssertThatContainerHasEnv(t, compassCont, "CLOUDSDK_COMPUTE_ZONE", "europe-west4-b")
	tester.AssertThatSpecifiesResourceRequests(t, actualJob.JobBase)
}

func TestKymaGKESeparateCompassIntegrationJobsReleases(t *testing.T) {
	for _, currentRelease := range releases.GetAllKymaReleases() {
		if currentRelease.IsNotOlderThan(releases.Release113) {
			t.Run(currentRelease.String(), func(t *testing.T) {
				...
				// THEN
				require.NoError(t, err)
				actualPresubmit := tester.FindPresubmitJobByNameAndBranch(jobConfig.AllStaticPresubmits([]string{"kyma-project/kyma"}), tester.GetReleaseJobName("kyma-gke-separate-compass-integration", currentRelease), currentRelease.Branch())
...
func TestKymaGKECompassIntegrationPostsubmit(t *testing.T) {
	...
	// when
	actualJob := tester.FindPostsubmitJobByNameAndBranch(jobConfig.AllStaticPostsubmits([]string{"kyma-project/kyma"}), "post-master-kyma-gke-separate-compass-integration", "master")
	require.NotNil(t, actualJob)
