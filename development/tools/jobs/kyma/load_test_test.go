...
func TestLoadTestJobsPresubmit(t *testing.T) {
	...
	assert.True(t, ex)
	assert.Len(t, kymaPresubmits, len(tester.GetAllKymaReleases())+1)
	...
	tester.AssertThatJobRunIfChanged(t, actualPresubmit, "tools/load-test/fix")
...
func TestLoadTestJobPostsubmit(t *testing.T) {
	// WHEN
	jobConfig, err := tester.ReadJobConfig("./../../../../prow/jobs/kyma/tools/load-test/load-test.yaml")
	// THEN
	require.NoError(t, err)
