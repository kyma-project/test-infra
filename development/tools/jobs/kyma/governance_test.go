...
func TestGovernanceJobPresubmit(t *testing.T) {
	...
	assert.Equal(t, []string{"kyma"}, actualPresubmit.Spec.Containers[0].Args)
}
...
func TestGovernanceJobPeriodic(t *testing.T) {
	...
	assert.Equal(t, "github.com/kyma-project/kyma", actualPeriodic.PathAlias)
	...
	assert.Equal(t, []string{"kyma", "true"}, actualPeriodic.Spec.Containers[0].Args)
}
