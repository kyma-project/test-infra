...
func TestGovernanceJobPeriodic(t *testing.T) {
	...
	tester.AssertThatHasExtraRefs(t, actualPeriodic.JobBase.UtilityConfig, []string{"test-infra", "kyma"})
	...
	repositoryDirArg := fmt.Sprintf("--repository-dir %s/kyma", tester.KymaProjectDir)
	assert.Equal(t, []string{"--repository kyma", repositoryDirArg, "--full-validation true"}, actualPeriodic.Spec.Containers[0].Args)
}
