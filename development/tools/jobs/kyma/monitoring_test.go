	...
	for _, currentRelease := range tester.GetAllKymaReleaseBranches() {
		...
		if currentRelease == "release-0.6" || currentRelease == "release-0.7" {
			...
			t.Run(currentRelease, func(t *testing.T) {
				...
				// THEN
				require.NoError(t, err)
				actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], tester.GetReleaseJobName("kyma-tests-monitoring", currençtRelease), currentRelease)
				require.NotNil(t, actualPresubmit)
				assert.False(t, actualPresubmit.SkipReport)
				assert.True(t, actualPresubmit.Decorate)
				assert.Equal(t, "github.com/kyma-project/kyma", actualPresubmit.PathAlias)
