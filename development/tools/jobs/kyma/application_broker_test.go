...
func TestApplicationBrokerReleases(t *testing.T) {
	for _, currentRelease := range tester.GetAllKymaReleaseBranches() {
		t.Run(currentRelease, func(t *testing.T) {
			...
			actualPresubmit := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-project/kyma"], tester.GetReleaseJobName("kyma-components-application-broker", currentRelease), currentRelease)
			...
			var imgVer string
			if tester.Release(currentRelease).Matches(tester.Release07, tester.Release08, tester.Release09, tester.Release10) {
