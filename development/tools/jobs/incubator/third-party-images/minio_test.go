...
const minioJobPath = "../../../../../prow/jobs/incubator/third-party-images/third-party-images.yaml"

func TestMinioJobsPresubmit(t *testing.T) {
	// when
	jobConfig, err := tester.ReadJobConfig(minioJobPath)

	// then
	require.NoError(t, err)
	actualPresubmit := tester.FindPresubmitJobByNameAndBranch(jobConfig.AllStaticPresubmits([]string{"kyma-incubator/third-party-images"}), "pre-master-tpi-minio", "master")
	...
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-incubator/third-party-images/minio", "ci-pr"}, actualPresubmit.Spec.Containers[0].Args)
}

func TestMinioJobPostsubmit(t *testing.T) {
	// when
	jobConfig, err := tester.ReadJobConfig(minioJobPath)
	...
	actualPostsubmit := tester.FindPostsubmitJobByNameAndBranch(jobConfig.AllStaticPostsubmits([]string{"kyma-incubator/third-party-images"}), "post-master-tpi-minio", "master")
	...
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-incubator/third-party-images/minio", "ci-master"}, actualPostsubmit.Spec.Containers[0].Args)
}
