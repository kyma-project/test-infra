...
func TestTerraformProviderGardenerJobsPresubmit(t *testing.T) {
	...
	kymaPresubmits, ex := jobConfig.Presubmits["kyma-incubator/terraform-provider-gardener"]
	assert.True(t, ex)
...
func TestTerraformProviderGardenerJobPostsubmit(t *testing.T) {
	...
	kymaPost, ex := jobConfig.Postsubmits["kyma-incubator/terraform-provider-gardener"]
	assert.True(t, ex)
	...
	tester.AssertThatHasPresets(t, actualPost.JobBase, tester.PresetDindEnabled, tester.PresetBuildMaster)
