...
func TestHealthcheckerJobReleases(t *testing.T) {
	...
	for _, currentRelease := range tester.GetKymaReleasesSince(tester.Release13) {
		t.Run(currentRelease.String(), func(t *testing.T) {
			...
			actualPre := tester.FindPresubmitJobByName(jobConfig.Presubmits["kyma-incubator/compass"], tester.GetReleaseJobName("compass-components-healthchecker", currentRelease), currentRelease.Branch())
