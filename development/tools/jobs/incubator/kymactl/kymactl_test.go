...
func TestServiceCatalogTesterJobsPresubmit(t *testing.T) {
	...
	kymaPresubmits, ex := jobConfig.Presubmits["kyma-incubator/kymactl"]
	assert.True(t, ex)
	...
	assert.Equal(t, "github.com/kyma-incubator/kymactl", actualPresubmit.PathAlias)
	assert.Equal(t, "^kymactl/", actualPresubmit.RunIfChanged)
	tester.AssertThatJobRunIfChanged(t, actualPresubmit, "kymactl/runner_worker.go")
...
func TestServiceCatalogTesterJobPostsubmit(t *testing.T) {
	...
	kymaPost, ex := jobConfig.Postsubmits["kyma-incubator/kymactl"]
	assert.True(t, ex)
	...
	assert.Equal(t, "github.com/kyma-incubator/kymactl", actualPost.PathAlias)
	...
	assert.Equal(t, []string{"/home/prow/go/src/github.com/kyma-incubator/kymactl"}, actualPost.Spec.Containers[0].Args)
}
