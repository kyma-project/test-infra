...
func TestVStudioExtensionJobsPresubmit(t *testing.T) {
	...
	kymaPresubmits, ex := jobConfig.Presubmits["kyma-incubator/vstudio-extension"]
	assert.True(t, ex)
	assert.Len(t, kymaPresubmits, 1)

	actualPresubmit := kymaPresubmits[0]
	expName := "vstudio-extension"
...
func TestVStudioExtensionJobPostsubmit(t *testing.T) {
	...
	kymaPost, ex := jobConfig.Postsubmits["kyma-incubator/vstudio-extension"]
	assert.True(t, ex)
