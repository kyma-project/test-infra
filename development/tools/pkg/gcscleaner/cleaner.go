...
func shouldDeleteBucket(
	...
	if err != nil {
		logrus.Debug(fmt.Sprintf("skipping bucket '%s', "+
			"error while parsing timestamp", bucketName))
		return false
	}
	if now-timestamp < int64(bucketLifespanDuration) {
		logrus.Debug(fmt.Sprintf(
			"bucket: '%s' is %s old and will not be deleted, "+
				"the duration: '%s' was not exceeded",
			bucketName,
