...
const defaultAddressRegexpList = "(remoteenvs-)?gkeint-(pr|commit)-.*,(remoteenvs-)?gke-upgrade-(pr|commit)-.*,(remoteenvs-)?(load-test|e2etest),gke-central-(pr|commit)-.*,gke-backup-(pr|commit)-.*,service-catalog-crd-periodic,gkecompint-(pr|commit)-.*,gke-minio-(pr)-.*"
