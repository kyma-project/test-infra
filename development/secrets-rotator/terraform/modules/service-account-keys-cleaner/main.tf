...
resource "google_cloud_scheduler_job" "service_account_keys_cleaner" {
  ...
  http_target {
    http_method = "GET"
    uri         = format("%s?project=%s&age=%s", google_cloud_run_service.service_account_keys_cleaner.status[0].url, var.project.id, var.service_account_key_latest_version_min_age)
