...
// Parent access management object.
...
func (projectsManager *projectsManager) getProject(projectname string) (project *Project) {
	ctx := context.Background()
	gkeproject, err := projectsManager.cloudresourcemanagerservice.Projects.Get(projectname).Context(ctx).Do()
	if err != nil {
...
func (project *Project) getPolicy() {
	...
	projectpolicy, err := project.projectsManager.cloudresourcemanagerservice.Projects.GetIamPolicy(project.gkeProject.Name, iampolicyrequest).Context(ctx).Do()
	if err != nil && !googleapi.IsNotModified(err) {
...
func (project *Project) setIamPolicy() {
	...
	_, err := project.projectsManager.cloudresourcemanagerservice.Projects.SetIamPolicy(project.gkeProject.Name, setiampolicyrequest).Context(ctx).Do()
	if err != nil {
