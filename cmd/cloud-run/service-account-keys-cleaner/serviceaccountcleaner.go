...
func serviceAccountKeysCleaner(w http.ResponseWriter, r *http.Request) {
	...
	if ok && len(keys[0]) > 0 {
		...
		if err != nil {
			gcphttp.WriteHTTPErrorResponse(w, http.StatusBadRequest, logger, "failed to convert age in hours to int: %s", err)
			return
		}
	}
	...
	if err != nil {
		gcphttp.WriteHTTPErrorResponse(w, http.StatusInternalServerError, logger, "failed to get all secrets from %s project", projectPath)
		return
	}
	...
	for _, secret := range secrets {
		...
		if err != nil {
			gcphttp.WriteHTTPErrorResponse(w, http.StatusInternalServerError, logger, "Could not get enabled versions of a %s secret: %s", secret.Name, err)
			return
		}
		...
		if err != nil {
			gcphttp.WriteHTTPErrorResponse(w, http.StatusInternalServerError, logger, "Couldn't parse date: %s", versions[0].CreateTime)
			return
		}
		...
		for i, version := range versions {
			...
			if err != nil {
				gcphttp.WriteHTTPErrorResponse(w, http.StatusInternalServerError, logger, "Couldn't get payload of a %s secret: %s", version.Name, err)
				return
			}
			...
			if err != nil {
				gcphttp.WriteHTTPErrorResponse(w, http.StatusInternalServerError, logger, "failed to unmarshal secret JSON field, error: %s", err)
				return
			}
