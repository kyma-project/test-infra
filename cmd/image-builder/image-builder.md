...
Image Builder binary now supports integration with the SRE Jenkins instance.
...
- **Build Report Path Flag**: A flag (`--build-report-path`) has been added to allow the tool to write a build report generated by the ADO
...
The Jenkins integration relies on a defined contract for sourcing input data via specific environment variables:
...
| `GIT_URL`         | Repository URL from which an image will be built. Must match the regex defined in [`extractOwnerAndRepoFromGitURL` function](https://github.com/kyma-project/test-infra/blob/df945b96654d60f82b9738cd98129191c5e753c8/cmd/image-builder/config.go#L123). | Always required                            |
