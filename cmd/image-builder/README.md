...
## Cross-Compiling for Multi-Architecture Images

When building images for multiple architectures, consider enabling cross-compilation to significantly reduce build times.
...
- **Cross-Compilation**: Implement cross-compilation in your Dockerfile, use
  ...
  as a reference.
- **Bind Mounts**: Use bind mounts for `RUN` command in Dockerfiles to avoid copying source code for compilation.
  Although the speed gain was minimal in our tests.
  We achived speed up of less than ~5 seconds.
- **Cache Strategy for Go Packages**: Use mounts of a cache type for Go package downloads and binary compilation cache, we rely on a **cache
  backed by a remote repository**. This is because a new agent is allocated for each pipeline execution, making mount-type caching
  ineffective.

### Applicability

These recommendations are especially beneficial for building images meant for multiple architectures, such as `linux/amd64` and
`linux/arm64`.
