...
import (
	...
	"github.com/kyma-project/test-infra/cmd/image-autobumper/autobumper"
...
var _ autobumper.PRHandler = (*client)(nil)
...
func parseOptions() (*options, *autobumper.Options, error) {
	var config string
...
func main() {
	...
	if err := autobumper.Run(ctx, pro, &client{o: o}); err != nil {
		logrus.WithError(err).Fatalf("failed to run the bumper tool")
	}
}
