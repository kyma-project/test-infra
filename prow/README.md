...
- Jobs enabled by creating and configuring the basic `config.yaml` file, an additional `yaml` file for a specific Kyma component in the `jobs` subfolder, and a related `Makefile` in the `kyma` repository. 
