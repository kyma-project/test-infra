...
gcloud container clusters create "${CLUSTER_NAME}" "${CLUSTER_VERSION_PARAM}" "${MACHINE_TYPE_PARAM}" "${NUM_NODES_PARAM}" --labels="job=kyma-gke-integration,job-id=${PROW_JOB_ID}"
