...
while [ ${SECONDS} -lt ${END_TIME} ];do
    ...
    log::date
    {
      ...
      kubectl --server kubernetes.default.svc --token "$token" --certificate-authority /var/run/secrets/kubernetes.io/serviceaccount/ca.crt get svc -n kube-system kube-dns -o jsonpath="{.spec.clusterIP}"
