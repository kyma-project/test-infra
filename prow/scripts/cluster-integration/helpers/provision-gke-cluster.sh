...
gcloud beta container clusters create "${CLUSTER_NAME}" "${CLUSTER_VERSION_PARAM}" "${MACHINE_TYPE_PARAM}" "${NUM_NODES_PARAM}" "${NETWORK_PARAM[@]}" "${LABELS_PARAM[@]}" ${STACKDRIVER_KUBERNETES_PARAM} ${DISK_TYPE_PARAM} ${REGIONAL_CLUSTER_PARAM}
