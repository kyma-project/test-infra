...
if [ "${PROVISION_REGIONAL_CLUSTER}" ] && [ "${CLOUDSDK_COMPUTE_REGION}" ] && [ "${NODES_PER_ZONE}" ];then REGIONAL_CLUSTER_PARAM_REGION="--region=${CLOUDSDK_COMPUTE_REGION}" REGIONAL_CLUSTER_PARAM_NODES="--num-nodes=${NODES_PER_ZONE}"; fi
...
gcloud beta container clusters create "${CLUSTER_NAME}" "${CLUSTER_VERSION_PARAM}" "${MACHINE_TYPE_PARAM}" "${NUM_NODES_PARAM}" "${NETWORK_PARAM[@]}" "${LABELS_PARAM[@]}" ${STACKDRIVER_KUBERNETES_PARAM} ${DISK_TYPE_PARAM} "${REGIONAL_CLUSTER_PARAM_REGION}" "${REGIONAL_CLUSTER_PARAM_NODES}"
