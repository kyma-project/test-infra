...
function reconciler::export_shoot_cluster_kubeconfig() {
  ...
  cat <<EOF | kubectl replace -f - --raw /apis/core.gardener.cloud/v1beta1/namespaces/garden-gardener-kyma-prow/shoots/${INPUT_CLUSTER_NAME}/adminkubeconfig | jq -r ".status.kubeconfig" | base64 -d > "${shoot_kubeconfig}"
{
