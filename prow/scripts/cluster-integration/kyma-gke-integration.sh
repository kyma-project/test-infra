...
if [[ "${GKE_RELEASE_CHANNEL}" == "rapid" ]]; then
  ...
  KYMA_TESTS="connector-service console-backend api-gateway application-connector monitoring console-web dex-connection apiserver-proxy kiali application-registry service-catalog cluster-users rafter connection-token-handler istio-kyma-validate logging dex-integration"
  export KYMA_TESTS
fi
