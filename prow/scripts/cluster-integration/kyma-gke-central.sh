...
# Description: Kyma Central plan on GKE. The purpose is to install and test Kyma on real GKE cluster in central mode.
#
#
# Expected vars:
#
...
function generateAndExportClusterName() {
    ...
    if [[ "$BUILD_TYPE" == "pr" ]]; then
        ...
        COMMON_NAME=$(echo "gke-central-pr-${PULL_NUMBER}-${RANDOM_NAME_SUFFIX}" | tr "[:upper:]" "[:lower:]")
        ...
        COMMON_NAME=$(echo "gke-central-rel-${RANDOM_NAME_SUFFIX}" | tr "[:upper:]" "[:lower:]")
    else
        # Otherwise (master), operate on triggering commit id
        COMMIT_ID=$(cd "$KYMA_SOURCES_DIR" && git rev-parse --short HEAD)
        COMMON_NAME=$(echo "gke-central-commit-${COMMIT_ID}-${RANDOM_NAME_SUFFIX}" | tr "[:upper:]" "[:lower:]")
    fi
