...
cleanup() {
    ...
    if [ -n "${CLEANUP_GATEWAY_DNS_RECORD}" ]; then
        ...
        date
        "${TEST_INFRA_CLUSTER_INTEGRATION_SCRIPTS}"/delete-dns-record.sh --project="${CLOUDSDK_CORE_PROJECT}" --zone="${CLOUDSDK_DNS_ZONE_NAME}" --name="${GATEWAY_DNS_FULL_NAME}" --address="${GATEWAYS_IP_ADDRESS}" --dryRun=false
    fi
