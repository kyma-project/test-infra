#!/usr/bin/env bash
# TODO: This is copy of kyma-upgrade-gke.sh and it's used to for prototyping compass upgrade on compass repo.
...
function addCompass() {
    #TODO: Use awk or sed or something similar to replace # -name|-namespace: "compass*"
    TMP_FILE=$1".tmp"
    FILE=$1
    COUNT=$(cat $FILE | wc -l)
    echo Lines: $COUNT
...
function testKyma() {
    shout "Test Kyma"
    date
    "${TEST_INFRA_SOURCES_DIR}"/prow/scripts/kyma-upgrade-compass-testing.sh
}
