...
### TODO: This file is checked out only temporary to validate if Octopus is mature enough to replace on Kyma repository
### helm test with Octopus
...
function generateAndExportSelfSignedCert() {
...
function installKyma() {
	...
	DOMAIN="${DNS_SUBDOMAIN}.${DNS_DOMAIN%?}"
	export DOMAIN
    generateAndExportSelfSignedCert
...
function installOctopus() {
  OCTOPUS_DIR="${KYMA_PROJECT_DIR}/../kyma-incubator/octopus/"
   helm install "${OCTOPUS_DIR}/chart/octopus" --name=octopus --wait
}
