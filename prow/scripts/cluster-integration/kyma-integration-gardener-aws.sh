...
# - MACHINE_TYPE (optional): AWS machine type
#
...
kyma provision gardener \
        ...
        --region "${GARDENER_REGION}" -z "${GARDENER_ZONES}" -t "${MACHINE_TYPE}" --disk-type gp2 --nodes 4 --kube-version=${GARDENER_CLUSTER_VERSION} \
)
