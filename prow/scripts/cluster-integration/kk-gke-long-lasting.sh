...
CLUSTER_NAME=$(echo "${INPUT_CLUSTER_NAME}" | tr "[:upper:]" "[:lower:]")
export CLUSTER_NAME

export GCLOUD_NETWORK_NAME="kk-gke-long-lasting-net"
export GCLOUD_SUBNET_NAME="kk-gke-long-lasting-subnet"
...
function installKyma() {
  ...
  # shellcheck disable=SC2043
	...
	KYMA_INSTALLER_IMAGE="${DOCKER_PUSH_REPOSITORY}${DOCKER_PUSH_DIRECTORY}/${CLUSTER_NAME}/${REPO_OWNER}/${REPO_NAME}:${CURRENT_TIMESTAMP}"
	...
	if [ -n "$(kubectl get service -n kyma-system apiserver-proxy-ssl --ignore-not-found)" ]; then
		...
		APISERVER_DNS_FULL_NAME="apiserver.${CLUSTER_NAME}.${DNS_DOMAIN}"
...
export DNS_DOMAIN
DOMAIN="${CLUSTER_NAME}.${DNS_DOMAIN%?}"
export DOMAIN
...
shout "Cleanup"
date
"${TEST_INFRA_CLUSTER_INTEGRATION_SCRIPTS}/kk-cleanup-cluster.sh"
