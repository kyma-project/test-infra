...
#TODO: this is a temporary file which is used for testing fluentd with stability-checker
...
function installStabilityChecker() {
    ...
    kubectl create secret generic sa-stability-fluentd-storage-writer --from-file=service-account.json=/etc/credentials/sa-stability-fluentd-storage-writer/service-account.json
