#!/usr/bin/env bash
#
# Execute eventing e2e pre upgrade tests on skr
#
#Description: Kyma CLI Integration plan on Gardener. This scripts implements a pipeline that consists of many steps.
# The purpose is to install and test Kyma using the CLI on a real Gardener cluster, run the eventing fast integration tests on skr cluster, upgrade the cluster to the current PR version and the run the tests again
#
...
log::info "Running skr eventing e2e upgrade with tests with SKR_SUFFIX: ${SKR_SUFFIX} and SKR_INSTANCE_ID: ${SKR_INSTANCE_ID}"
