...
function utils::compress_send_to_vm() {
  ...
  gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --strict-host-key-checking=no --quiet --zone="${ZONE}" --command="mkdir ${REMOTE_PATH} && tar -xf ~/pack.tar.gz -C ${REMOTE_PATH}" --ssh-flag="-o ServerAliveInterval=30" "${REMOTE_NAME}"
