...
cleanup() {
    ...
    utils::receive_from_vm "${ZONE}" "busola-ui-test-${RANDOM_ID}" "${HOME}/busola-tests/cypress/screenshots" "${ARTIFACTS}"
    utils::receive_from_vm "${ZONE}" "busola-ui-test-${RANDOM_ID}" "${HOME}/busola-tests/cypress/videos" "${ARTIFACTS}"
