...
if [[ -z "${PULL_NUMBER}" ]]; then
    LABELS=(--labels "branch=$PULL_BASE_REF,job-name=busola-integration-test-docker")
else
    LABELS=(--labels "pull-number=$PULL_NUMBER,job-name=busola-integration-test-docker")
fi
