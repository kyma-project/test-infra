...
cleanup() {
    ...
    utils::receive_from_vm "${ZONE}" "busola-smoke-test-${RANDOM_ID}" "~/busola-tests/cypress/screenshots" "${ARTIFACTS}"
    #shellcheck disable=SC2088
...
for ZONE in ${EU_ZONES}; do
    ...
    gcloud compute instances create "busola-smoke-test-${RANDOM_ID}" \
    --metadata enable-oslogin=TRUE \
    --image "${IMAGE}" \
    --machine-type n2-highcpu-16 \
    --zone "${ZONE}" \
    --boot-disk-size 200 "${LABELS[@]}" && \
    log::info "Created busola-smoke-test-${RANDOM_ID} in zone ${ZONE}" && break
...
#shellcheck disable=SC2088
utils::send_to_vm "${ZONE}" "busola-smoke-test-${RANDOM_ID}" "${TMP_DIR}/kubeconfig-${KYMA_CLUSTER_NAME}.yaml" "~/kubeconfig-kyma.yaml"
...
log::info "Launching the busola-smoke-test-docker.sh script"
# gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --verbosity="${GCLOUD_SSH_LOG_LEVEL:-error}" --quiet --zone="${ZONE}" --command="sudo bash" --ssh-flag="-o ServerAliveInterval=30" "busola-smoke-test-${RANDOM_ID}" < "${SCRIPT_DIR}/cluster-integration/busola-smoke-test-docker.sh"

log::info "SKIPPED - TODO"

log::info "now push image pls"
