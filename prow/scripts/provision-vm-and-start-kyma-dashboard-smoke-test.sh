...
cleanup() {
    ...
    utils::receive_from_vm "${ZONE}" "kyma-dashboard-smoke-test-${RANDOM_ID}" "~/kyma-dashboard-tests/cypress/screenshots" "${ARTIFACTS}"
    #shellcheck disable=SC2088
    utils::receive_from_vm "${ZONE}" "kyma-dashboard-smoke-test-${RANDOM_ID}" "~/kyma-dashboard-tests/cypress/videos" "${ARTIFACTS}"
    
    gcloud compute instances delete --zone="${ZONE}" "kyma-dashboard-smoke-test-${RANDOM_ID}"
...
if [[ -z "${PULL_NUMBER}" ]]; then
    LABELS=(--labels "branch=$PULL_BASE_REF,job-name=kyma-dashboard-smoke-test")
else
    LABELS=(--labels "pull-number=$PULL_NUMBER,job-name=kyma-dashboard-smoke-test")
fi
...
for ZONE in ${EU_ZONES}; do
    ...
    gcloud compute instances create "kyma-dashboard-smoke-test-${RANDOM_ID}" \
    --metadata enable-oslogin=TRUE \
    --image "${IMAGE}" \
    --machine-type n2-highcpu-16 \
    --zone "${ZONE}" \
    --boot-disk-size 200 "${LABELS[@]}" && \
    log::info "Created kyma-dashboard-smoke-test-${RANDOM_ID} in zone ${ZONE}" && break
...
cat <<EOF | kubectl replace -f - --raw /apis/core.gardener.cloud/v1beta1/namespaces/garden-kyma-prow/shoots/${KYMA_CLUSTER_NAME}/adminkubeconfig | jq -r ".status.kubeconfig" | base64 -d > "${TMP_DIR}/kubeconfig-${KYMA_CLUSTER_NAME}.yaml"
{
...
#shellcheck disable=SC2088
utils::send_to_vm "${ZONE}" "kyma-dashboard-smoke-test-${RANDOM_ID}" "${TMP_DIR}/kubeconfig-${KYMA_CLUSTER_NAME}.yaml" "~/kubeconfig-kyma.yaml"

log::info "Copying Kyma-Dashboard 'tests' folder to the instance"
#shellcheck disable=SC2088
utils::compress_send_to_vm "${ZONE}" "kyma-dashboard-smoke-test-${RANDOM_ID}" "/home/prow/go/src/github.com/kyma-project/kyma-dashboard/tests" "~/kyma-dashboard-tests"
