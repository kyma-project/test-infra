...
if [[ -v TELEMETRY_ENABLED ]] || [[ -v TELEMETRY_MANAGED_FLUENT_BIT_ENABLED ]]; then
  ...
  utils::ssh_to_vm_with_script -z "${ZONE}" -n "kyma-integration-test-${RANDOM_ID}" -c "sudo bash" -p "${SCRIPT_DIR}/cluster-integration/kyma-integration-k3d-telemetry-managed-fluent-bit.sh"
