...
if [ "$KUBERNETES_RUNTIME" = 'minikube' ]; then
    ...
    # install k3d v5.0.0
    gcloud compute ssh --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" -- "yes | sudo wget -q -O - https://raw.githubusercontent.com/rancher/k3d/main/install.sh | TAG=v5.0.0 bash"
