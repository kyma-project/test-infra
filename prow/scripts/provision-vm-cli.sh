...
gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --no-user-output-enabled --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" --command="mkdir \$HOME/bin"
...
gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --no-user-output-enabled --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" --command="sudo cp \$HOME/bin/kyma /usr/local/bin/kyma"
...
if [ "$KUBERNETES_RUNTIME" = 'minikube' ]; then
    gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --no-user-output-enabled --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" --command="yes | sudo kyma provision minikube --non-interactive"
else
    gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --no-user-output-enabled --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" --command="yes | sudo kyma provision k3d --ci"
fi
...
if [ "$KUBERNETES_RUNTIME" = 'k3d' ]; then
    gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --no-user-output-enabled --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" --command="yes | sudo kyma deploy --ci ${SOURCE}"
else
    gcloud compute ssh --ssh-key-file="${SSH_KEY_FILE_PATH:-/root/.ssh/user/google_compute_engine}" --no-user-output-enabled --quiet --zone="${ZONE}" "cli-integration-test-${RANDOM_ID}" --command="yes | sudo kyma install --non-interactive ${SOURCE}"
fi
