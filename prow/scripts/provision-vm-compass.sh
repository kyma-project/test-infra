...
gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "compass-integration-test-${RANDOM_ID}" -- "mkdir \$HOME/bin"

#shellcheck disable=SC2088
utils::send_to_vm "${ZONE}" "compass-integration-test-${RANDOM_ID}" "kyma" "~/bin/kyma"

gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "compass-integration-test-${RANDOM_ID}" -- "sudo cp \$HOME/bin/kyma /usr/local/bin/kyma"
...
gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "compass-integration-test-${RANDOM_ID}" -- "yes | ./compass/installation/scripts/prow/deploy-and-test.sh ${DUMP_DB}"
