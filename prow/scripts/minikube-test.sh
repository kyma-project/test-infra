...
set +e
gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "kyma-integration-test-${RANDOM_ID}" -- ./kyma/installation/scripts/prow/kyma-integration-on-debian/deploy-and-test-kyma.sh
RUN_RESULT="$?"
set -e
