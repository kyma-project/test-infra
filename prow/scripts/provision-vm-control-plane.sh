...
gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "control-plane-integration-test-${RANDOM_ID}" -- "mkdir \$HOME/bin"

#shellcheck disable=SC2088
utils::send_to_vm "${ZONE}" "control-plane-integration-test-${RANDOM_ID}" "kyma" "~/bin/kyma"

gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "control-plane-integration-test-${RANDOM_ID}" -- "sudo cp \$HOME/bin/kyma /usr/local/bin/kyma"

log::info "Triggering the installation"

gcloud compute ssh --verbosity="${GCLOUD_SSH_LOG_LEVEL}" --quiet --zone="${ZONE}" "control-plane-integration-test-${RANDOM_ID}" -- "yes | ./control-plane/installation/scripts/prow/deploy-and-test.sh"
