...
cleanup() {
    ARG=$?
    shout "Removing instance control-plane-integration-test-${RANDOM_ID}"
...
for ZONE in ${EU_ZONES}; do
    shout "Attempting to create a new instance named control-plane-integration-test-${RANDOM_ID} in zone ${ZONE} using image ${IMAGE}"
    ...
    shout "Created control-plane-integration-test-${RANDOM_ID} in zone ${ZONE}" && break
    shout "Could not create machine in zone ${ZONE}"
done || exit 1

trap cleanup exit INT

shout "Copying control-plane to the instance"
