...
utils::ssh_to_vm_with_script -z "${ZONE}" -n "serverless-manager-test-${RANDOM_ID}" -c 'sudo bash -c "source /etc/profile && apt-get update && sleep 120 && apt-get install git -y && sleep 120 && git version && cd $HOME/serverless-manager && make test"'
