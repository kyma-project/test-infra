...
utils::ssh_to_vm_with_script -z "${ZONE}" -n "serverless-manager-test-${RANDOM_ID}" -c 'sudo bash -c "source /etc/profile && apt get update && apt get install git && git version && cd $HOME/serverless-manager && make test"'
