...
utils::ssh_to_vm_with_script -z "${ZONE}" -n "serverless-manager-test-${RANDOM_ID}" -c 'sudo bash -c "source /etc/profile && sudo apt-get update && sleep 20 && sudo apt-get upgrade -y && sudo apt-get install git -y && git version && cd $HOME/serverless-manager && make test"'
