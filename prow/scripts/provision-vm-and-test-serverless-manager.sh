...
utils::ssh_to_vm_with_script -z "${ZONE}" -n "serverless-manager-test-${RANDOM_ID}" -c 'sudo bash -c "source /etc/profile && sudo apt-get update && sleep 120 && sudo apt-get upgrade && sleep 120 && sudo apt-get install git -y && sleep 120 && git version && cd $HOME/serverless-manager && make test"'
