...
utils::ssh_to_vm_with_script -z "${ZONE}" -n "serverless-manager-test-${RANDOM_ID}" -c 'sudo bash -c "source /etc/profile && apt-get install libgit2-dev libssh2-1-dev libssl-dev -y && cd $HOME/serverless-manager && make test"'
