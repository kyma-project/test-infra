...
utils::ssh_to_vm_with_script -z "${ZONE}" -n "serverless-manager-test-${RANDOM_ID}" -c 'sudo bash -c "source /etc/profile && sudo killall apt apt-get && apt-get update && apt-get install git -y && git version && cd $HOME/serverless-manager && make test"'
