...
func TestNewServicePrincipalCredential(t *testing.T) {
	tests := []struct {
		name    string
		cfg     ServicePrincipalConfig
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cred, err := NewServicePrincipalCredential(tt.cfg)
			if (err != nil) != tt.wantErr {
				t.Errorf("NewServicePrincipalCredential() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr && cred == nil {
				t.Error("NewServicePrincipalCredential() returned nil credential without error")
			}
		})
	}
}
...
func TestServicePrincipalProvider_GetToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider := NewServicePrincipalProvider(tt.mock)
